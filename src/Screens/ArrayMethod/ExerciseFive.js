...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 //1)The first account has nested arrays of tags in it's tag property.  Get a list of all tags for this account as a one-dimensional array.
  const ListofAccountOneDimension = accounts[0].tags[7][0]
  console.log(ListofAccountOneDimension)

  //Output--> deserunt
  //Now time to get the items with string form(Which are nested conditions)
  const listOfTagItems = [].concat.apply([],accounts[0].tags)
  console.log(listOfTagItems)

  //Output--> ["nostrud", "aliquip", "est", "laboris", "voluptate", "do", "dolor", "deserunt", "ipsum", "consectetur", "quis", "commodo"]
  
  //we can also used for the same result flat()
  const ForsameResult = accounts[0].tags.flat()
  console.log(ForsameResult)
  ...
  const tagForTheFirstAccount = accounts[0].tags.includes('consectetur')
  console.log('The result we get',tagForTheFirstAccount)
 //Output--> The result we get false

  // Here we can see consectetur are inside tags[] but it doesn't show and give wrong result

  const correction = accounts[0].tags.flat().includes('consectetur')
  console.log('We get the correct result',correction)

  //Output--> We get the correct result true

  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//3)Get a list of all the tags for all the accounts and join them together as a string separated by commas.

const TagsAllStringSeparated = accounts.map(({tags})=> tags.join(',')).join(',')
console.log('Sari Strings aa gayi',TagsAllStringSeparated)

//Output--> Sari Strings aa gayi nostrud,aliquip,est,laboris,voluptate,do,dolor,deserunt,ipsum,consectetur,quis,commodo,et,ut,mollit,occaecat,ipsum,mollit,pariatur,laboris,reprehenderit,est,in,duis,voluptate,non,consequat,non,dolore,cupidatat,irure,ipsum,quis,et,minim,sunt,irure,et,esse,consectetur
  
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

//4)Get a list of all the friends for all the user's accounts as an array (don't worry about duplicates)

 const ListofFriends = accounts.map(({friends})=> friends.map(({name})=> name)).flat()
 console.log('We get the friends list',ListofFriends)
 //Output-->We get the friends list ["Alisa Ford", "Hess Howell", "Gwendolyn Hendrix", "Miranda Little", "Ellen Burks", "Shanna Logan", "Mayer Sullivan", "Wiggins Hudson", "Jacqueline Gibbs", "Isabelle Luna", "Marissa Beach", "Sarah Simmons", "Cynthia Hays", "Estela Walton", "Christy Ball"]


 //5)Get a list of all the friends for all users who are younger than 30 AND have a balance more than $1500
  
 const getResult = accounts.filter(({balance,age})=> age<= 30 &&  balance > parseInt(balance.replace(/\D/g,'' ))> 1500).map((({friends})=> friends)).flat()
 console.log('Get all the list of friends', getResult)
 
  return (
    <View>
      <Text>ExerciseFive</Text>
    </View>
  )
}
